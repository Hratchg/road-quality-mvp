...
| 7-XX-XX | TBD | 0 | REQ-trained-la-detector (SC #4) | T-02-01 (pickle-ACE) | `_DEFAULT_HF_REPO` starts with `hratcho/road-quality-la-yolov8@` (revision-pinned) | unit | `pytest backend/tests/test_detector_factory.py::test_default_hf_repo_pin -x` | ⚠️ W0 update | ⬜ pending |
...
| 7-XX-XX | TBD | N | REQ-trained-la-detector (SC #3) | T-02-01 (pickle-ACE) | Model published to HF at `hratcho/road-quality-la-yolov8@<sha>` | manual smoke | `python -c "from huggingface_hub import HfApi; print(HfApi().model_info('hratcho/road-quality-la-yolov8').sha)"` | N/A | ⬜ pending |
...
- [ ] `backend/tests/test_detector_factory.py` — update `test_default_hf_repo_pin` to verify `_DEFAULT_HF_REPO` starts with `hratcho/road-quality-la-yolov8@` after Phase 7 constant update. Covers SC #4 + Pitfall 8 (pickle-ACE).
...
| Model published to `hratcho/road-quality-la-yolov8@<sha>` | SC #3 | HF Hub network call; not in CI | `python -c "from huggingface_hub import HfApi; print(HfApi().model_info('hratcho/road-quality-la-yolov8').sha)"` |
