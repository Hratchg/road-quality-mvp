...
Source ~10× more LA Mapillary imagery, hand-label to ≥150 positive bboxes (vs 17 today), fine-tune YOLOv8 on the new dataset, *measurably* beat the Phase 6 public-baseline (P=0.143, R=0.333), publish the trained model to HuggingFace at `hratcho/road-quality-la-yolov8@<sha>`, re-ingest production with the trained detector, and replace the baseline numbers in `docs/DETECTOR_EVAL.md`.
...
5. HF publish at `hratcho/road-quality-la-yolov8` with pinned revision SHA
...
- `data_pipeline/detector_factory.py::_DEFAULT_HF_REPO` — single line update after publish: `hratcho/road-quality-la-yolov8@<new_sha>`
